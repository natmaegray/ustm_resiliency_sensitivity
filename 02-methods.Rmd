...
The MC random sampling uses the R function of *rnorm*, such as:
```
...
```
LHS uses the *lhs* package in R. Since this package only chooses variables on a zero to one scale, the values given the use the following function to put the random sampling on the right scale needed for the given parameter. 
```
...
100 and 600 draws of random samples are taken, the distributions for the HBW parameters are shown in Figure \@ref(fig:parameter100) and Figure \@ref(fig:parameter600) respectively. These distributions show that LHS gives more normally distributed parameters with less draws than MC sampling.
